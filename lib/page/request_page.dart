...
class _SubmitCubit extends Cubit<_SubmitState> {
  ...
  Future<void> submit({
    ...
    emit(_SubmitLoading());
    try {
      final col = FirebaseFirestore.instance.collection('request_demo');
      final doc = col.doc();
      await doc.set({
        'id':     doc.id,
        'gender': gender,
...
class _FormScreenState extends State<_FormScreen> {
  ...
  // text type    → String (free text)
  // dropdown type → String (value key from CMS values list)
  ...
  void _syncDynControllers() {
    for (final q in widget.model.demoQuestions.questions) {
              ...
              child: Column(
                ...
                children: [
                  ...
                  if ((isAr
                      ...
                      .isNotEmpty)
                    Text(
                      isAr ? m.header.title.ar : m.header.title.en,
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Validation row for country/city
                        ...
                        if (m.demoQuestions.questions.isNotEmpty) ...[
                          SizedBox(height: 20.h),
                          Divider(color: _C.border, height: 1),
                          SizedBox(height: 20.h),
                          _sectionTitle(isAr
                          ...
                          SizedBox(height: 12.h),
                          ...List.generate(
                            m.demoQuestions.questions.length,
                                (i) {
                              final q = m.demoQuestions.questions[i];
                              return Padding(
                                padding: EdgeInsets.only(
                                  bottom: i <
                                      m.demoQuestions.questions.length - 1
                                      ? 12.h
                                      : 0,
                                ),
  ...
  Widget _sectionTitle(String t) => Text(
    t,
    style: StyleText.fontSize14Weight600.copyWith(color: _C.section),
  );
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(
          ...
          child: Row(
            children: [
              // Code picker styled like the other fields
              Container(
                height: 36.h,
        ...
        // Reserve the same height as CustomValidatedTextFieldMaster error lane
  ...
  Widget _dynQuestion(DemoQuestionModel q, bool isAr) {
    ...
    // DROPDOWN type — uses CustomDropdownFormFieldInvMaster
...
class _ConfirmScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isAr = Localizations.localeOf(context).languageCode == 'ar';
    final c     = model.confirmMessage;
    final title = (isAr ? c.title.ar : c.title.en).isNotEmpty
        ? (isAr ? c.title.ar : c.title.en)
        : 'Waiting till Customer Services Call You';
    final desc  = (isAr ? c.description.ar : c.description.en).isNotEmpty
